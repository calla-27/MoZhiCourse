    ...
    <div class="container">
        ...
        <div class="profile-info">
          ...
          <div class="profile-quick-stats">
            ...
            <div class="quick-stat">
              ...
              <span class="stat-num">{{ user.learningStats?.favorites_count || collectList.length || 0 }}</span>
      ...
      <section v-if="activeTab === 'analysis'" class="content-section">
        ...
        <div class="stats-grid">
          <div class="stat-card"><div class="value">{{ user.learningStats?.total_learning_hours || 0 }}min</div><div class="label">总学习时长</div></div>
      ...
      <section v-if="activeTab === 'library'" class="content-section">
        ...
        <div class="courses-grid">
          <div v-for="course in libraryList" :key="course.id" class="course-card">
            <div class="course-image" :style="{backgroundImage: course.bg || 'linear-gradient(135deg,#a8edea,#fed6e3)'}"></div>
      ...
      <section v-if="activeTab === 'collect'" class="content-section">
        ...
        <div class="courses-grid">
          <div v-for="course in collectList" :key="course.id" class="course-card">
            <div class="course-image" :style="{backgroundImage: course.bg || 'linear-gradient(135deg,#ffecd2,#fcb69f)'}"></div>
            <div class="course-content">
              <h3 class="course-title">{{ course.title || course.name }}</h3>
              <p>{{ course.desc || course.description }}</p>
      ...
      <section v-if="activeTab === 'community'" class="content-section">
        ...
        <div class="community-stats">
          <div class="stat-card">
            <div class="value">{{ communityData.teams?.length || 0 }}</div>
            <div class="label">活跃组队</div>
          </div>
          <div class="stat-card">
            <div class="value">{{ communityData.rooms?.length || 0 }}</div>
            <div class="label">自习室</div>
          </div>
          <div class="stat-card">
            <div class="value">{{ communityData.postStats?.total_posts || 0 }}</div>
            <div class="label">讨论帖子</div>
          </div>
          <div class="stat-card">
            <div class="value">{{ (communityData.postStats?.total_likes || 0) + (communityData.postStats?.total_comments || 0) }}</div>
        ...
        <div class="community-section">
          ...
          <div class="teams-grid" v-if="communityData.teams && communityData.teams.length > 0">
            <div v-for="team in communityData.teams" :key="team.team_id" class="team-card">
              <div class="team-header">
                <h4 class="team-name">{{ team.team_name }}</h4>
                <span class="team-status active">进行中</span>
              </div>
              <p class="team-desc">{{ team.description || '一起学习，共同进步' }}</p>
              <div class="team-members">
                <div class="member-avatars">
                  <div class="member-avatar">{{ team.team_name.charAt(0) }}</div>
                  <div class="member-avatar" v-if="team.current_members > 1">+{{ team.current_members - 1 }}</div>
                </div>
                <span class="member-count">{{ team.current_members }}/{{ team.max_members }}人</span>
              </div>
              <div class="team-progress">
                <div class="progress-bar">
                  <div class="progress-fill" :style="{width: Math.round((team.current_members / team.max_members) * 100) + '%'}"></div>
                </div>
                <span class="progress-text">{{ Math.round((team.current_members / team.max_members) * 100) }}% 满员</span>
              </div>
              <div class="team-actions">
                <button class="action-btn primary" @click="goToTeam(team.team_id)">进入小组</button>
                <button class="action-btn" v-if="team.role === 'leader'">管理</button>
              </div>
            </div>
          </div>
          <div v-else class="empty-state">
            ...
            <p>还没有加入任何学习小组</p>
            <p class="empty-hint">加入学习小组，与志同道合的伙伴一起学习</p>
            <button class="browse-btn" @click="goToCommunity">探索小组</button>
          </div>
        </div>
...
<div class="community-section">
  ...
  <div class="teams-grid" v-if="communityData.rooms && communityData.rooms.length > 0">
    <div v-for="room in communityData.rooms" :key="room.room_id" class="team-card">
      <div class="team-header">
        <h4 class="team-name">{{ room.room_name }}</h4>
        <span class="team-status active">进行中</span>
      </div>
      <p class="team-desc">{{ room.description || '专注学习的安静空间' }}</p>
      <div class="team-members">
        <div class="member-avatars">
          <div class="member-avatar">{{ room.room_name.charAt(0) }}</div>
          <div class="member-avatar" v-if="room.current_participants > 1">+{{ room.current_participants - 1 }}</div>
        </div>
        <span class="member-count">{{ room.current_participants }}/{{ room.max_participants }}人</span>
      </div>
      <div class="team-progress">
        <div class="progress-bar">
          <div class="progress-fill" :style="{width: Math.round((room.current_participants / room.max_participants) * 100) + '%'}"></div>
        </div>
        <span class="progress-text">{{ Math.round((room.current_participants / room.max_participants) * 100) }}% 满员</span>
      </div>
      <div class="team-actions">
        <button class="action-btn primary" @click="goToRoom(room.room_id)">进入自习室</button>
        <button class="action-btn" v-if="room.role === 'host'">管理</button>
      </div>
    </div>
  </div>
  <div v-else class="empty-state">
    ...
    <p>还没有加入任何自习室</p>
    <p class="empty-hint">加入自习室，与小伙伴们一起专注学习</p>
    <button class="browse-btn" @click="goToCommunity">探索自习室</button>
  </div>
</div>
      </section>
...
const goToRoom = (roomId) => {
  router.push(`/community/rooms/${roomId}`)
}
  ...
  try {
    ...
    if (data.success) {
      libraryList.value = data.data.map(course => ({
        ...
        description: course.course_desc || course.description || '暂无描述',
...
const getCourseCover = (course) => {
  ...
  if (coverImg) {
    ...
    if (coverImg.startsWith('http')) {
      console.log('   ✅ 使用完整URL:', coverImg)
      return `url("${coverImg}")`
    } else if (coverImg.startsWith('/')) {
      const fullUrl = `${API_BASE}${coverImg}`
      console.log('   ✅ 拼接相对路径:', fullUrl)
      return `url("${fullUrl}")`
    } else {
      const fullUrl = `${API_BASE}/uploads/${coverImg}`
      console.log('   ✅ 拼接uploads路径:', fullUrl)
      return `url("${fullUrl}")`
    }
  }
  
  console.log('   ⚠️ 无封面图片，使用默认渐变')
  // 使用默认渐变背景
...
const loadCommunityData = async () => {
  try {
    const token = getToken()
    console.log('🏘️ 加载社区数据，调用接口:', `${API_BASE}/api/personal/community`)
    const res = await fetch(`${API_BASE}/api/personal/community`, {
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
    })

    if (!res.ok) {
      console.warn('社区数据接口返回错误状态:', res.status)
      communityData.value = { teams: [], rooms: [], postStats: { total_posts: 0, total_likes: 0, total_comments: 0 } }
      return
    }

    const data = await res.json()
    console.log('🏘️ 社区数据响应:', data)
    
    if (data && data.success) {
      communityData.value = {
        teams: data.data.teams || [],
        rooms: data.data.rooms || [],
        postStats: data.data.postStats || { total_posts: 0, total_likes: 0, total_comments: 0 }
      }
      console.log(`🏘️ 加载到 ${communityData.value.teams.length} 个学习小组，${communityData.value.rooms.length} 个自习室`)
    } else {
      communityData.value = { teams: [], rooms: [], postStats: { total_posts: 0, total_likes: 0, total_comments: 0 } }
    }
  } catch (error) {
...
const handleAvatarUpdate = async (avatarUrl) => {
  console.log('🎨 收到头像更新通知:', avatarUrl)
  
  // 更新用户store中的头像
  await user.updateAvatar(avatarUrl)
  
  // 强制刷新页面显示
  console.log('✅ 头像已更新，当前头像URL:', user.avatarUrl)
}
