      ...
      <div class="charts-row">
        <div class="chart-container chart-half">
          <div class="chart-header">
            <h3><i class="fas fa-chart-pie"></i> 学生知识掌握分布</h3>
          </div>
          <div ref="masteryChartRef" class="chart-content"></div>
        </div>
        <div class="chart-container chart-half">
          <div class="chart-header">
            <h3><i class="fas fa-cloud"></i> 学习行为词云</h3>
          </div>
          <div ref="wordCloudChartRef" class="chart-content"></div>
        </div>
      </div>
    </section>
...
/* ---------- 图表相关 ---------- */
const masteryChartRef = ref(null)    // 环状图DOM引用
const wordCloudChartRef = ref(null)  // 词云图DOM引用
let masteryChart = null              // ECharts实例
let wordCloudChart = null            // ECharts实例
const masteryData = ref([])          // 掌握分布数据
const wordCloudData = ref([])        // 词云数据
...
function initMasteryChart() {
  ...
  // 处理数据，添加颜色
  const chartData = masteryData.value.map(item => ({
    ...item,
...
// 监听 activeTab 切换到 analytics 时初始化图表
