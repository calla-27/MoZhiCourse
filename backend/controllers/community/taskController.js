...
const taskController = {
    ...
    try {
        ...
        const { creator_id, title, description, deadline } = req.body;

        console.log('创建任务请求:', { teamId, creator_id, title, deadline });
        ...
        const [result] = await execute(
            'INSERT INTO team_task (team_id, creator_id, title, description, deadline) VALUES (?, ?, ?, ?, ?)',
            [teamId, creator_id, title, description, mysqlDeadline]
        );
    ...
    updateTask: async (req, res) => {
        try {
            ...
            const [result] = await execute(
                'UPDATE team_task SET title = ?, description = ?, deadline = ? WHERE task_id = ?',
