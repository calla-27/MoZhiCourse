...
class Course {
  ...
  static async search(keyword) {
    // 标签关键词映射 - 将标签转换为更容易匹配的关键词
    const tagKeywordMap = {
      '考公务员': ['考公', '公务员', '申论', '行测'],
      '考研': ['考研', '研究生', '考研英语', '考研数学'],
      '四六级': ['四级', '六级', '英语四级', '英语六级', 'CET'],
      '教师资格证': ['教师', '教资', '教师资格'],
      '会计师': ['会计', 'CPA', '注会'],
      '建造师': ['建造师', '一建', '二建'],
      '法考': ['法考', '司法考试', '法律'],
    ...
    if (tagKeywordMap[keyword]) {
      searchKeywords = tagKeywordMap[keyword];
    }
    
    // 构建WHERE条件
    const whereConditions = searchKeywords.map(() => 
      '(c.course_name LIKE ? OR c.course_desc LIKE ?)'
    ).join(' OR ');
    
    // 构建参数数组
    const params = searchKeywords.flatMap(kw => [`%${kw}%`, `%${kw}%`]);
    ...
    const [courses] = await pool.execute(
      `SELECT 
       ...
       LEFT JOIN course_chapter cc ON cc.course_id = c.course_id
       LEFT JOIN course_video cv ON cv.chapter_id = cc.chapter_id
       WHERE (${whereConditions}) AND c.is_online = 1
       GROUP BY c.course_id
       ORDER BY (c.cover_img IS NOT NULL AND c.cover_img <> '') DESC,
                (video_count > 0) DESC,
                c.student_count DESC,
