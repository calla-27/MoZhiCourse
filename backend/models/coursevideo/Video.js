...
class Video {
  ...
  static async updateProgress(userId, videoId, progress, currentTime, duration = 0) {
    try {
      ...
      if (!courseId) {
        console.error(`❌ 未找到视频对应的课程ID: videoId=${videoId}`);
        throw new Error('未找到视频对应的课程');
      }

      // 计算学习时长（基于进度变化）
      const learnDuration = Math.max(1, Math.floor(duration * (progress / 100) / 60)); // 转换为分钟

      const [result] = await pool.execute(
        `INSERT INTO learning_detail 
         (user_id, course_id, video_id, learn_duration, complete_rate, current_position, learn_time) 
         VALUES (?, ?, ?, ?, ?, ?, NOW())`,
        [userId, courseId, videoId, learnDuration, progress, currentTime]
      );
      
      console.log(`✅ 进度更新成功，插入ID: ${result.insertId}, 进度: ${progress}%`);
      
      // 同时更新用户课程表的进度和学习时长
      try {
        // 计算累计学习时长（从learning_detail表汇总）
        const [durationResult] = await pool.execute(
          `SELECT SUM(learn_duration) as total_duration
           FROM learning_detail 
           WHERE user_id = ? AND course_id = ?`,
          [userId, courseId]
        );
        
        const totalDuration = durationResult[0]?.total_duration || 0;
        
        await pool.execute(
          `INSERT INTO user_course (user_id, course_id, progress, total_learn_duration, last_learn_time, is_enrolled)
           VALUES (?, ?, ?, ?, NOW(), 1)
           ON DUPLICATE KEY UPDATE 
           progress = GREATEST(progress, VALUES(progress)),
           total_learn_duration = ?,
           last_learn_time = NOW()`,
          [userId, courseId, progress, totalDuration, totalDuration]
        );
        console.log(`✅ 用户课程进度和学习时长已同步更新: 进度${progress}%, 时长${totalDuration}分钟`);
      } catch (courseError) {
        console.log(`⚠️ 更新用户课程进度失败:`, courseError.message);
      }
  ...
  static async recordBehavior(userId, videoId, behaviorData) {
    ...
    try {
      ...
      // 3. 如果是播放/暂停/完成/学习时长行为，也记录到learning_detail表
      if (behaviorType === 'play' || behaviorType === 'pause' || behaviorType === 'complete' || behaviorType === 'learning_duration') {
        try {
          // 计算学习时长
          let learnDuration = 1; // 默认1分钟
          
          if (behaviorType === 'complete') {
            learnDuration = Math.floor(duration / 60); // 完成时使用完整时长
          } else if (behaviorType === 'learning_duration' && behaviorData.sessionDuration) {
            learnDuration = Math.floor(behaviorData.sessionDuration / 60); // 使用实际学习时长
          }
          ...
          console.log(`✅ 学习数据已更新到learning_detail表，时长: ${learnDuration}分钟`);
          
          // 同步更新user_course表的总学习时长
          if (actualCourseId) {
            try {
              const [durationResult] = await pool.execute(
                `SELECT SUM(learn_duration) as total_duration
              ...
              await pool.execute(
                `INSERT INTO user_course (user_id, course_id, total_learn_duration, last_learn_time, is_enrolled)
                 VALUES (?, ?, ?, NOW(), 1)
                 ON DUPLICATE KEY UPDATE 
                 total_learn_duration = ?,
                 last_learn_time = NOW()`,
                [userId, actualCourseId, totalDuration, totalDuration]
              );
              
              console.log(`✅ 用户课程总学习时长已更新: ${totalDuration}分钟`);
            } catch (updateError) {
              console.log('⚠️ 更新用户课程总时长失败:', updateError.message);
            }
          }
