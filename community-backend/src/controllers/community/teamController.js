...
const teamController = {
    ...
    getAvailableData: async (req, res) => {
        try {
            ...
            const [users] = await execute(
                'SELECT user_id, user_name, role, email FROM t_user WHERE is_active = 1 ORDER BY user_name'
            );
            
            // 获取所有课程
            const [courses] = await execute(
                'SELECT course_id, course_name, teacher_user_id FROM t_course ORDER BY course_name'
            );
            
            // 获取所有标签（从现有小组中提取）
            const [tagsResult] = await execute(
                'SELECT DISTINCT tags FROM t_study_team WHERE tags IS NOT NULL'
            );
    ...
    createTeam: async (req, res) => {
        try {
            ...
            const [userExists] = await execute(
                'SELECT user_id, user_name, role FROM t_user WHERE user_id = ? AND is_active = 1',
                [creator_id]
            );
            ...
            const [teamExists] = await execute(
                'SELECT team_id FROM t_study_team WHERE team_name = ? AND status = "active"',
                [team_name]
            );
    ...
    deleteTeam: async (req, res) => {
        try {
            ...
            // 1. 检查小组是否存在
            const [teamExists] = await execute(
                'SELECT team_id, creator_id FROM t_study_team WHERE team_id = ?',
                [teamId]
            );
            ...
            // 3. 先删除相关的任务记录
            await execute(
                'DELETE FROM t_team_task WHERE team_id = ?',
                [teamId]
            );

            // 4. 删除小组成员记录
            await execute(
                'DELETE FROM t_team_member WHERE team_id = ?',
                [teamId]
            );

            // 5. 删除小组
            const [result] = await execute(
                'DELETE FROM t_study_team WHERE team_id = ?',
                [teamId]
            );
