...
const messageController = {
    ...
    createMessage: async (req, res) => {
        try {
            ...
            const [roomExists] = await execute(
                'SELECT room_id FROM t_study_room WHERE room_id = ? AND status = "open"',
                [roomId]
            );
            ...
            const [messages] = await execute(`
                ...
                FROM t_room_message rm
                LEFT JOIN t_user u ON rm.user_id = u.user_id
                WHERE rm.message_id = ?
            `, [result.insertId]);
    ...
    try {
        ...
        const [countResult] = await execute(
            'SELECT COUNT(*) as total FROM t_room_message WHERE room_id = ?',
            [roomId]
        );
    ...
    deleteMessage: async (req, res) => {
        try {
            ...
            const [result] = await execute(
                'DELETE FROM t_room_message WHERE message_id = ?',
                [messageId]
            );
