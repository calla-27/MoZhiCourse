    ...
    createComment: async (req, res) => {
        try {
            ...
            const [postExists] = await execute(
                'SELECT post_id, title, status, post_type FROM t_community_post WHERE post_id = ?',
                [post_id]
            );
            ...
            // 如果是QA类型的帖子，评论自动成为回答
            if (post.post_type === 'qa_help' || post.post_type === 'qa_teacher') {
            ...
            if (parent_comment_id) {
                const [parentCommentExists] = await execute(
                    'SELECT comment_id FROM t_post_comment WHERE comment_id = ?',
                    [parent_comment_id]
                );
                ...
                const [parentCommentPost] = await execute(
                    'SELECT post_id FROM t_post_comment WHERE comment_id = ?',
                    [parent_comment_id]
                );
            ...
            const [duplicateComment] = await execute(
                'SELECT comment_id FROM t_post_comment WHERE post_id = ? AND user_id = ? AND content = ?',
            ...
            // 9. 更新帖子的评论数
            await execute(
                'UPDATE t_community_post SET comment_count = comment_count + 1 WHERE post_id = ?',
                [post_id]
            );
            ...
            res.status(201).json({
                ...
                data: newComment  // 直接返回评论对象，而不是包装对象
            });
...
getPostComments: async (req, res) => {
    try {
        const { postId } = req.params; // 注意这里是 postId
    ...
    updateComment: async (req, res) => {
        try {
            ...
            const [comment] = await execute(
                'SELECT user_id FROM t_post_comment WHERE comment_id = ?',
                [commentId]
            );
            ...
            const [result] = await execute(
                'UPDATE t_post_comment SET content = ? WHERE comment_id = ?',
                [content, commentId]
            );
    ...
    deleteComment: async (req, res) => {
        try {
            ...
            const [commentExists] = await execute(
                'SELECT comment_id, user_id, post_id, comment_type FROM t_post_comment WHERE comment_id = ?',
                [commentId]
            );
            ...
            try {
                ...
                await connection.execute(
                    'DELETE FROM t_comment_like WHERE comment_id = ?',
                    [commentId]
                );

                // 删除评论
                const [deleteResult] = await connection.execute(
                    'DELETE FROM t_post_comment WHERE comment_id = ?',
                    [commentId]
                );
                ...
                // 更新帖子评论数
                await connection.execute(
                    'UPDATE t_community_post SET comment_count = GREATEST(comment_count - 1, 0) WHERE post_id = ?',
                    [postId]
                );

                // 获取更新后的评论数
                const [post] = await connection.execute(
                    'SELECT comment_count FROM t_community_post WHERE post_id = ?',
                    [postId]
                );
                ...
                console.log('评论删除成功，已更新帖子评论数:', { 
                    commentId, 
                    postId, 
                    newCommentCount: post[0].comment_count 
                });

                res.json({
                    success: true,
                    message: comment.comment_type === 'qa_answer' ? '回答删除成功' : '评论删除成功',
                    data: {
                        post_id: postId,
    ...
    likeComment: async (req, res) => {
        try {
            ...
            console.log('点赞评论请求:', { commentId, user_id });
            ...
            const [existingLike] = await execute(
                'SELECT like_id FROM t_comment_like WHERE comment_id = ? AND user_id = ?',
                [commentId, user_id]
            );
            ...
            try {
                ...
                // 更新评论点赞数
                await connection.execute(
                    'UPDATE t_post_comment SET like_count = like_count + 1 WHERE comment_id = ?',
                    [commentId]
                );

                // 获取更新后的点赞数
                const [updatedComment] = await connection.execute(
                    'SELECT like_count FROM t_post_comment WHERE comment_id = ?',
                    [commentId]
                );
                ...
                console.log('评论点赞成功:', { 
                    commentId, 
                    user_id, 
                    newLikeCount: updatedComment[0].like_count 
                });
                ...
                res.json({
                    ...
                    data: {
                        ...
                        like_count: updatedComment[0].like_count,
                        user_has_liked: true
                    }
                });
    ...
    unlikeComment: async (req, res) => {
        try {
            ...
            console.log('取消点赞评论请求:', { commentId, user_id });
            ...
            try {
                // 删除点赞记录
                const [deleteResult] = await connection.execute(
                    'DELETE FROM t_comment_like WHERE comment_id = ? AND user_id = ?',
                    [commentId, user_id]
                );
                ...
                // 更新评论点赞数
                await connection.execute(
                    'UPDATE t_post_comment SET like_count = GREATEST(like_count - 1, 0) WHERE comment_id = ?',
                    [commentId]
                );
    ...
    checkCommentLike: async (req, res) => {
        try {
            ...
            console.log('检查评论点赞状态:', { commentId, user_id });
            ...
            res.json({
                ...
                data: {
                    ...
                    has_liked: hasLiked,
                    like_count: commentInfo[0] ? commentInfo[0].like_count : 0
                }
            });
