...
const qaPostController = {
    ...
    createTeacherQuestion: async (req, res) => {
        try {
            ...
            const [result] = await execute(
                'INSERT INTO t_community_post (user_id, post_type, title, content, course_id, tags) VALUES (?, "qa_teacher", ?, ?, ?, ?)',
    ...
    createHelpPost: async (req, res) => {
        try {
            ...
            // 创建帖子
            const [result] = await execute(
                'INSERT INTO t_community_post (user_id, post_type, title, content, course_id, tags) VALUES (?, "qa_help", ?, ?, ?, ?)',
                [user_id, title, content, course_id, JSON.stringify(tags || [])]
            );
    ...
    getQAPosts: async (req, res) => {
        try {
            ...
            let query = `
                ...
                WHERE cp.status = 'normal' AND cp.post_type = ?
            `;
    ...
    deleteQAPost: async (req, res) => {
        try {
            ...
            const [postExists] = await execute(
                'SELECT post_id, user_id, post_type FROM t_community_post WHERE post_id = ?',
                [postId]
            );
            ...
            // 检查帖子类型是否为QA类型
            if (!['qa_teacher', 'qa_help'].includes(post.post_type)) {
                return res.status(400).json({
                    success: false,
                    message: '只能删除问答类型的帖子'
                });
            ...
            // 删除相关的评论
            await execute(
                'DELETE FROM t_post_comment WHERE post_id = ?',
                [postId]
            );

            // 删除相关的点赞
            await execute(
                'DELETE FROM t_post_like WHERE post_id = ?',
                [postId]
            );

            // 删除帖子
            const [result] = await execute(
                'DELETE FROM t_community_post WHERE post_id = ?',
                [postId]
            );
