    ...
    getTeacherQA: async (req, res) => {
        try {
            ...
            let query = `
                ...
                WHERE cp.status = 'normal' AND cp.post_type = 'qa_teacher'
            `;
    ...
    getHelpPosts: async (req, res) => {
        try {
            ...
            let query = `
                ...
                LEFT JOIN t_user u ON cp.user_id = u.user_id
                LEFT JOIN t_course c ON cp.course_id = c.course_id
                WHERE cp.status = 'normal' AND cp.post_type = 'qa_help'
            `;
    ...
    markAsSolved: async (req, res) => {
        try {
            ...
            const [result] = await execute(
                'UPDATE t_community_post SET status = "closed" WHERE post_id = ?',
                [postId]
            );
