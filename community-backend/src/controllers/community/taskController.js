...
const taskController = {
    ...
    try {
        ...
        const { creator_id, task_title, task_content, deadline } = req.body;

        console.log('创建任务请求:', { teamId, creator_id, task_title, deadline });

        // 1. 基础字段验证
        if (!creator_id || !task_title) {
        ...
        const [teamExists] = await execute(
            'SELECT team_id, team_name, status FROM t_study_team WHERE team_id = ?',
            [teamId]
        );
        ...
        const [creatorInTeam] = await execute(
            'SELECT member_id FROM t_team_member WHERE team_id = ? AND user_id = ? AND status = "active"',
            [teamId, creator_id]
        );
        ...
        // 4. 验证任务标题长度
        if (task_title.length < 2 || task_title.length > 200) {
            return res.status(400).json({
                success: false,
        ...
        const [result] = await execute(
            ...
            [teamId, creator_id, task_title, task_content, mysqlDeadline]
        );
    ...
    updateTask: async (req, res) => {
        try {
            const { taskId } = req.params;
            const { task_title, task_content, deadline } = req.body;

            const [result] = await execute(
                'UPDATE t_team_task SET task_title = ?, task_content = ?, deadline = ? WHERE task_id = ?',
                [task_title, task_content, deadline, taskId]
            );
    ...
    deleteTask: async (req, res) => {
        try {
            ...
            const [result] = await execute(
                'DELETE FROM t_team_task WHERE task_id = ?',
                [taskId]
            );
