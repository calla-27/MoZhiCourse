...
class StudyTeam {
    ...
    static async findAll(filters = {}) {
        let query = `
            ...
            LEFT JOIN t_user u ON st.creator_id = u.user_id
            LEFT JOIN t_team_member tm ON st.team_id = tm.team_id AND tm.status = 'active'
    ...
    static async update(teamId, updateData) {
        const [result] = await execute(
            'UPDATE t_study_team SET team_name = ?, description = ?, max_members = ?, tags = ? WHERE team_id = ?',
